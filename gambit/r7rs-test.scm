...
(define (test-r7rs)
  ...
  ;(test> (list (unless #t 43) (unless #f 42)) equal? '(#f 42))
...
(define (proc-test)
  (let ((go? (##inline-host-statement "g_host2scm(GambitProcessor);")))
    (##inline-host-statement "console.error(@1@)" go?)
    (if go? (test-r7rs) (proc-test))))
