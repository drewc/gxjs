...
** Initialize Modules after registry::  No longer, archive me
:PROPERTIES:
:CUSTOM_ID: gambitModInit
:END:

To lazy load our ~js~ *Gambit* modules the ~RTS.module_register~ function has been
...
#+begin_src javascript :noweb-ref RTS.mod_init
RTS.module_registry_reset = function () {
  const r = this;
  r.module_count = 0;
  r.module_map = {};
  r.module_table = null;
  r.module_latest_registered = null;
};

if (RTS.old_module_registry_init === undefined) {
  RTS.old_module_registry_init = RTS.module_registry_init;
};

RTS.module_registry_init = function (link_info) {
  this.module_registry_reset();
  this.old_module_registry_init(link_info);
  console.log('init module registry', link_info);
};

RTS.module_name = function (module_descr) {
  var temp = module_descr[0];
  var name = temp[temp.length - 1].name;
  return name;
};

RTS.module_init = function(module_descr) {
  const r = this;
  r.sp = -1;
  r.stack[++this.sp] = void 0;
  r.r0 = this.underflow;
...
if (RTS.old_module_register === undefined) {
  RTS.old_module_register = RTS.module_register;
};

RTS.module_register = function (module_descr) {
  this.all_modules.push(module_descr);
  // console.log('Registered a module', module_descr);
  if ( typeof this.glo['##program-descr'] === 'object' ) {
    this.module_init(module_descr);
  } else {
    this.old_module_register(module_descr);
  }
};
...
sync require for browser: https://gist.github.com/serapath/e2b55cab315e60fbbffea7b43acd8749
  https://stackoverflow.com/questions/13225245/require-js-synchronous-loading
#+begin_src javascript :noweb-ref RTS.lazy

RTS.lazy = function (prop, setter) {
  const glo = this.glo
  Object.defineProperty(this.glo, prop, {
    configurable: true,
    get: function () {
      ...
      var nprop = false;
      var nval;
      const prom = setter(RTS, prop);

      Promise.resolve(prom).then((val) => { nprop = true; nval = val });

      //while (nprop === false) {
        console.log('promise Value?', nprop, prop)
        //setTimeout(function(){ nprop = true}, 500);

       //}
    globalThis.prom = () => [nprop, nval];
      console.log('New Value?', prom, glo[prop], prop)
      return glo[prop];
    },
    set: function (value) {
      Object.defineProperty(this, prop, {
...
RTS.lazy('go4it', () => {
  return import(/* webpackChunkName: "print" */ 'gxjs-module!gxjs-arrow?args=["RTS"]!gxjs-var!gxjs-min!gsc!/gambit/lib/list.scm'
  )
    .then(module => { console.log(module); module.default(RTS) });
})

;; RTS.lazy('make-vector', (glo, p) => { console.log('get!!', glo, p); })

#+end_src

#+begin_src javascript :tangle ../lazy.js

function lazyList(RTS, prop) {
  var ret = import(/* webpackChunkName: "list" */ 'gxjs-module!gxjs-arrow?args=["RTS"]!gxjs-var!gxjs-min!gsc!/gambit/lib/list.scm')
    .then(module => {
      module.default(RTS);
      console.log('loaded', RTS.glo, prop, RTS.glo['member'])
      var ret = RTS.glo[prop];
      return true;
    })
  return ret;
...
module.exports = function lazy(RTS) {

  ['member', 'assoc'].map((s) =>  RTS.lazy(s, () => lazyList(RTS, s)))
